...
class robot:
    ...
    def move(self, motion, tolerance=0.001):  # Do not change the name of this function
        ...
        if alpha > max_steering_angle:
            raise ValueError("Steering exceeds maximum steering angle")
        if d < 0.0:
        ...
        # Case where turning radius is infinity (beta is small)
        if (abs(beta) < tolerance):

            # Compute new position and orientation of the robot
            x_prime = self.x + d * cos(self.orientation)
            y_prime = self.y + d * sin(self.orientation)
            orientation_prime = (self.orientation + beta) % (2 * pi)

        # Finite radius of rotation
        else:
            # Turning radius R
            R = d / beta

            # Compute center of rotation cx and cy
            cx = self.x - R * sin(self.orientation)
            cy = self.y + R * cos(self.orientation)

            # Compute new position and orientation of the robot
            x_prime = cx + R * sin(self.orientation + beta)
            y_prime = cy - R * cos(self.orientation + beta)
            orientation_prime = (self.orientation + beta) % (2 * pi)
    ...
    def sense(self, noise=1):  # do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            Z.append(bearing + bearing_noise)
